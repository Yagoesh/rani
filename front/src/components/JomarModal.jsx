...
import JomarForm from "./JomarForm";

function JomarModal() {
  const [allJomar, setAllJomar] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const getAllJomarApi = async () => {
    const data = await axios.get(`http://localhost:4000/jomar/all`);
    setAllJomar(data.data.data.result);
  };
  ...
  return (
    <div className="text-center">
      <Toaster />
      <h2 className="text-2xl mb-4">Jomar</h2>
      ...
      <JomarForm isOpen={isModalOpen} onClose={closeModal} />
    </div>
  );
}

export default JomarModal;
