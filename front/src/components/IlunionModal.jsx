...
function IlunionModal() {
  ...
  const getAllIlunionApi = async () => {
    const data = await axios.get(`http://localhost:4000/ilunion/all`);
    setAllIlunion(data.data.data.result);
  };
  ...
  return (
    <div className="text-center">
      ...
      <IlunionForm isOpen={isModalOpen} onClose={closeModal} />
    </div>
  );
}

export default IlunionModal;
